...
function createVoiceHandler(_voice, _polyphony, _stealMode){
  ...
  return {
    ...
    get applyDynamicPatchChange(){return function(_newVals){
      activeVoices.forEach((_voice, _index) => {
        _voice.applyDynamicPatchChange(_newVals);
      });
    }},
...
function loadAudioModule(_context){
  _context.audioWorklet.addModule('libs/sound_engine/audio_processors.js').then(() => {
    modulesLoaded = true;
